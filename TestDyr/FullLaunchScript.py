...
def ProcessManager(option):
    for i in range(2,8):
        ...
        save = open(output, "w")
        parameters = ["prefixSpan.exe", "Dataset3(4Var).csv", "100%", str(option), str(i)]
...
def ProcessManager1(option):
    for i in range(2,5):
        qualifier = "inclusion (28:5)+ " + option + " " + str(i)
        output = qualifier + ".txt"
        save = open(output, "w")
        parameters = ["prefixSpan.exe", "Dataset3(4Var).csv", "100%", "-include", "28:5",str(option), str(i)]
        p = subprocess.Popen( parameters, stdin=subprocess.PIPE, stdout=save)
        try:
            Wait_timeout(p, gTimeout)
            save.close()
...
def ProcessManager2(option):
    for i in range(2,5):
        qualifier = "inclusion (28:5)+ -minSseq 3 + " + option + " " + str(i)
        output = qualifier + ".txt"
        save = open(output, "w")
        parameters = ["prefixSpan.exe", "Dataset3(4Var).csv", "100%", "-include", "28:5", "-minSseq", "3",str(option), str(i)]
        p = subprocess.Popen( parameters, stdin=subprocess.PIPE, stdout=save)
        try:
            Wait_timeout(p, gTimeout)
            save.close()
...
##        proc.join()   
##    for option in options:
##        proc = multiprocessing.Process(target=ProcessManager1, args=[option])
##        proc.start()
##        procList.append(proc)
##    for proc in procList:
##        proc.join()
    options = ["-minSize", "-maxSize", "-maxSseq"]
    for option in options:
        proc = multiprocessing.Process(target=ProcessManager2, args=[option])
        proc.start()
        procList.append(proc)
    for proc in procList:
        proc.join()
